...
VAE model with heirarchical priors to promote disentanglement
"""
...
class DenseVAE_HP_Dis(nn.Module):
    def __init__(self, data_channels, initial_features, denseblocks, growth_rate, n_latent, activations = nn.ReLU()):
        ...
        super(DenseVAE_HP_Dis, self).__init__()
        ...
        self.prior_enc_logvar = nn.Parameter(torch.zeros(self.n_latent))
        self.prior_dec_logvar = nn.Parameter(torch.zeros(self.n_latent))
    ...
    def forward(self, x):
        ...
        ximu, xilogvar = self.prior_enc(z), self.prior_enc_logvar
        xi = self._reparameterize(ximu, xilogvar)
        
        zpmu, zplogvar = self.prior_dec(xi), self.prior_dec_logvar
    ...
    def compute_loss(self, x):
        ...
        return zmu, zlogvar, z, xmu, xlogvar, ximu, xilogvar, xi, zpmu, zplogvar, l_rec, l_reg
    
    def forward_w_rot(self, x, At):
        zmu, zlogvar = self.enc_mean_network(x), self.enc_logvar(x)
        #zmu = torch.mm(zmu, At)
        #zlogvar = torch.mm(zmu, At))
        z = torch.mm(self._reparameterize(zmu, zlogvar), At)
        
        ximu, xilogvar = self.prior_enc(z), self.prior_enc_logvar
        xi = self._reparameterize(ximu, xilogvar)
        
        zpmu, zplogvar = self.prior_dec(xi), self.prior_dec_logvar
        xmu, xlogvar = self.dec_mean_network(torch.mm(z,torch.transpose(At, 0, 1))), self.dec_logvar #
        return zmu, zlogvar, z, xmu, xlogvar, ximu, xilogvar, xi, zpmu, zplogvar
    
    def compute_loss_w_rot(self, x, At):
        # At is the transpose of a rotation matrix
        n_mc = 9
        zmu, zlogvar, z, xmu, xlogvar, ximu, xilogvar, xi, zpmu, zplogvar = self.forward_w_rot(x, At)
        l_rec = -torch.sum(self.gaussian_log_prob(x, xmu, xlogvar), 1)
        #print('sum_log_prob...:', torch.sum(self.gaussian_log_prob(z, zpmu, zplogvar), 1)
        h = torch.sum(self.gaussian_log_prob(torch.mm(z, torch.transpose(At, 0, 1)), zmu, zlogvar), 1)#-self.n_latent/2*(1+ np.log(2*math.pi)) - 0.5*torch.sum(zlogvar)
        klp = torch.sum(self.compute_kld(ximu, xilogvar), 1)
        glp = torch.sum(self.gaussian_log_prob(z, zpmu, zplogvar), 1)
        
        for i in range(n_mc):
            z = torch.mm(self._reparameterize(zmu, zlogvar), At)        
            ximu = self.prior_enc(z)
            xi = self._reparameterize(ximu, xilogvar)
            zpmu = self.prior_dec(xi)
            xmu = self.dec_mean_network(torch.mm(z, torch.transpose(At, 0, 1)))
            l_rec = l_rec - torch.sum(self.gaussian_log_prob(x, xmu, xlogvar), 1)
            h = h + torch.sum(self.gaussian_log_prob(torch.mm(z, torch.transpose(At, 0, 1)), zmu, zlogvar), 1)
            klp = klp + torch.sum(self.compute_kld(ximu, xilogvar), 1)
            glp = glp + torch.sum(self.gaussian_log_prob(z, zpmu, zplogvar), 1)
